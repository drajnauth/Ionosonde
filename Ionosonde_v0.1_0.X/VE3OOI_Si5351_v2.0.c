...
void setupSi5351 (int correction)
{
  ...
  Si5351WriteRegister (SIREG_3_OUTPUT_ENABLE_CTL, 0xFF);  // Each bit corresponds to a clock output.  1 to disable, 0 to enable
...
//base = Si5351ReadRegister (SIREG_3_OUTPUT_ENABLE_CTL);
//printf ("Setup: %x\r\n", base);  
}
...
// This routine turns off CLKs by setting the corresponding bit in the CLK control register
{
  // Disable CLK0, CLK1, CLK2
    
  // Assumes clkenable keeps a running config of state of all clocks
  switch (clk) {
    case SI_CLK0:
...
//base = Si5351ReadRegister (SIREG_3_OUTPUT_ENABLE_CTL);
//printf ("Enable Before: %x\r\n", (unsigned int)base);  
  Si5351WriteRegister (SIREG_3_OUTPUT_ENABLE_CTL, clkenable);
//base = Si5351ReadRegister (SIREG_3_OUTPUT_ENABLE_CTL);
//printf ("Enable After: %x\r\n", (unsigned int)base);  
...
#ifdef DEBUG_PRINT
  sendString ("MS: ");
  sendDecNumber (freq_temp);
  sendString (" ");
  sendDecNumber (pllfreq);
  sendString (" ");
  fraction = (double)MS_a+(double)MS_b/(double)MS_c;
  fraction = (double)pllfreq / fraction;
  sendDecNumber ( (unsigned long)fraction );
  sendString (" ");
  sendDecNumber ( ((double)MS_a+(double)MS_b/(double)MS_c));
  sendString (" ");
  sendDecNumber (MS_a);
  sendString (" ");
  sendDecNumber (MS_b);
...
void Si5351RepeatedWriteRegister(unsigned char  addr, unsigned char  bytes, unsigned char *data)
{
    ...
    for (i=0; i<bytes; i++){
        Si5351WriteRegister ((char)(addr+i), data[i]);
    }
...
// Routine uses the I2C protocol to write data to the Si5351 register.
{

  if (I2C_WriteReg(SI5351_ADDRESS, reg, value) == I2C_ERROR) {
    sendString ("I2C W Err ");
  }
}
...
// This function uses I2C protocol to read data from Si5351 register. The result read is returned
{
  ...
  if (I2C_ReadReg(SI5351_ADDRESS, reg, (char *)&value) == I2C_ERROR) {
    sendString ("I2C R Err ");
    value = 0;
  }
